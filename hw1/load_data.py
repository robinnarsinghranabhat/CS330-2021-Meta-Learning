    ...
    def sample_batch(self, batch_type, batch_size):
        ...
        for b in range(batch_size):
            ...
            for ind, class_folder in enumerate(sampled_character_classes):
                ...
                sampled_images_per_character_path = random.sample(
                    os.listdir(class_folder), self.num_samples_per_class + 1
                )
                sampled_images_per_character_path = [
                    os.path.join(class_folder, i)
                    for i in sampled_images_per_character_path
                ]

                sampled_images_per_character = [
                    image_file_to_array(i, self.dim_input)
                    for i in sampled_images_per_character_path
                ]
            ...
            class_labels = np.split(
                class_labels, self.num_samples_per_class + 1, axis=1
            )
        ...
        return (
            torch.tensor(sampled_input, dtype=torch.float).to(self.device),
            torch.tensor(sampled_labels, dtype=torch.float).to(self.device),
        )
