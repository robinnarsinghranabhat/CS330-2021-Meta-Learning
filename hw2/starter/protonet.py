...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # My implementation maintains certain expectation for the input
            assert all(torch.sort(labels_support)[0] == labels_support)
            assert all(torch.sort(labels_query)[0] == labels_query)

            ## 1. ====== compute the prototypes ======= ##
            # NOTE : DO WE BATCH NORMALIZE OVER THE ENTIRE TRAIN/TEST Period
            # compact_representation = self._network(torch.cat( (images_support, images_query), dim=0  ) )
            # support_rep, query_rep = torch.split( compact_representation, [ len(images_support), len(images_query)  ], dim=0 )

            # TODO: OR, Should we do instead Something like , too tired to think now
            support_rep = self._network(images_support)
            query_rep = self._network(images_query)

            # Our class prototypes
            class_prototypes = []
            _, counts = np.unique(labels_support, return_counts=True)
            classwise_support = torch.split(support_rep, counts.tolist())
            for i in classwise_support:
                class_prototypes.append(torch.mean(i, dim=0))

            class_prototypes = torch.stack(class_prototypes)

            ## Each Row : distance of nth query from each of classes
            support_distance = torch.negative(
                torch.cdist(support_rep, class_prototypes, p=2)
            )
            support_logits = nn.functional.softmax(support_distance, dim=1)

            query_distance = torch.negative(
                torch.cdist(query_rep, class_prototypes, p=2)
            )
            query_logits = nn.functional.softmax(query_distance, dim=1)

            ## ====== and the protonet loss ====== ##
